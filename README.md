...
Equivalent** is the matching field in pyModeS's 1090 `decode()` output. A ❌ in either column means that library
has nothing there -- pyModeS-only fields (no ❌ in the pyModeS Equivalent column) are inserted alphabetically as
if pyModeS978 had a field of that same name, so the whole table reads as one merged, alphabetical list rather
than "ours first, then theirs."
...
| `emergency` | `emergency_state` (bare `int`) | `Emergency` enum — emergency/priority status. Name doesn't match pyModeS's `emergency_state` — an oversight from #29's field-parity pass, not a deliberate choice; tracked in [#46](https://github.com/BrentIO/pyModeS978/issues/46). | Payload types 1, 3. |
